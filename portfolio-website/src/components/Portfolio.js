...
const Portfolio = () => {
  ...
  const onTouchMove = (e) => {
    if (!selectedImage) return; // Only prevent default when modal is open
    e.preventDefault();
              ...
              >
                {images.map((image) => (
                  <m.div
                    ...
                    className='relative aspect-[3/4] rounded-lg overflow-hidden shadow-lg cursor-pointer group'
                  ...
                  >
                    <img
                      ...
                      loading='lazy'
                    />
                    <div className='absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-active:opacity-100 transition-opacity duration-200' />
                  </m.div>
                ))}
              </m.div>
            ) : (
        ...
        <AnimatePresence>
          {selectedImage && (
            <m.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className='fixed inset-0 z-[200] isolate bg-black/90 p-4 flex items-center justify-center'
              style={{ touchAction: selectedImage ? 'none' : 'auto' }}
              onClick={() => setSelectedImage(null)}
              onTouchStart={selectedImage ? onTouchStart : undefined}
              onTouchMove={selectedImage ? onTouchMove : undefined}
              onTouchEnd={selectedImage ? onTouchEnd : undefined}
            >
              <button
              ...
              {/* Swipe indicators - subtle edge indicators */}
              <AnimatePresence>
                {swipeDirection === 'left' && (
                  <m.div
